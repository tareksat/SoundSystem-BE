...
cron.schedule('* * * * *', async() => {
    ...
    console.log('Check if there is a prayer time now every minute', moment().format('YYYY-MM-DD HH:mm:ss'));
});

// every hour to update today's prayer times
cron.schedule('0 0 */1 * * *', async() => {
    await AzanServices.fetchPrayerTimes();
    console.log('running a task every hour', moment().format('YYYY-MM-DD HH:mm:ss'));
});
